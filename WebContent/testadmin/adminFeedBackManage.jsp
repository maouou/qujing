...
function handleReport(id)
{
	    ...
	    url = "${pageContext.request.contextPath}/admin/feedbackManage/showdetail.do?id=" + id;//弹出窗口的页面内容
	    var oOpen = window.open(url,"adviceDetail",model);
	    oOpen.focus();
}
function handleReport1(id)
{
	location.href = "${pageContext.request.contextPath}/admin/feedbackManage/showdetail.do?id=" + id;
}
</script>
    ...
    <div class="listDataTableDiv">
        <table
            ...
            <c:forEach items="${feedBacks}" var="f">
                <tr>
                    <td>${f.task.name}</td>                
                    <td>${f.task.receiverid}</td>
                    <td>${f.task.senderid}</td>
                    <td>${f.type.type}</td>
                    <td>${f.content}</td>
                    <td><a onclick="handleReport1(${f.id})">处理</a></td>
                </tr>
			</c:forEach>
            </tbody>
        </table>
