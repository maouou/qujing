...
function deleteTask(id)
{
	...
	{
		location.href="${pageContext.request.contextPath}/admin/reportmanage/deletetask.do?id=" + id;
	}
}
function turnBack()
{
	location.href="${pageContext.request.contextPath}/admin/reportmanage/list.do";
}
function noHandle(id)
{
	if(confirm("确定该任务未违规吗？"))
	{
		location.href="${pageContext.request.contextPath}/admin/reportmanage/legaltask.do?id=" + id;
	}
}
function turnToUserManage(id)
{
	...
	{
		location.href="${pageContext.request.contextPath}/admin/reportmanage/turnusermanage.do?id=" + id;
	}
...
<div>
	<%
		Suit suit = (Suit)request.getAttribute("suit");
	...
	%>
	<div id="taskname">
	任务名：<h2><%=suit.getTask().name %></h2>
	</div>
	<div id="taskcontent">
	任务描述：<h2><%=suit.getTask().content %></h2>
	</div>
	<div id="receiver">
	收货人：<h2><%=suit.getTask().receiverid %>   <%=receiver.username %></h2>
	</div>
	<div id="sender">
	送货人：<h2><%=suit.getTask().senderid %>   <%=sender.username %></h2>
	</div>
	<div id="cause">
	举报理由：<h2><%=suit.getContent() %></h2>
	</div>
	<div id="btn_deletetask">
	<button onclick="deleteTask(<%=suit.getId() %>)">删除任务</button>
	</div>
	<!-- 跳转进行用户处理 -->
	<!--  <div id="btn_usermanage">
	<button onclick="turnToUserManage(<%=suit.getId() %>)">用户管理</button>
	</div> -->
	<!-- 页面内嵌用户处理 -->
	<div id="btn_usermanage">
	...
	<div id="btn_nohandle">
	<button onclick="noHandle(<%=suit.getId() %>)"> 未违规 </button>
	</div>
	<div id="btn_goback">
	...
	<div id="changeUserPoints" hidden="true" style="position:absolute;
		...
		border:3px solid;
		margin:0 20px">
		<form id="changePointsForm" method="post" action="${pageContext.request.contextPath}/admin/reportmanage/openusermanage.do?receiverIDNumber=<%=receiver.IDNumber %>&senderIDNumber=<%=sender.IDNumber %>&id=<%=suit.getId() %>">
